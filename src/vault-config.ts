...
export function getChainVaults(): Array<VaultConfig> {
  ...
  // default config for when we need the time weighted investor balance
  const bb = new TrackingFeatureFlags(ONE_BI, ONE_BI, ONE_BI, ZERO_BI)
  ...
  if (network === "linea") {
    const POINTS_TRACKING_START_BLOCK = BigInt.fromI32(3174336) // first vault we want to track time weighted values for
    ...
    const all = new TrackingFeatureFlags(
      POINTS_TRACKING_START_BLOCK,
      POINTS_TRACKING_START_BLOCK,
      POINTS_TRACKING_START_BLOCK,
      LINEA_TVL_TRACKING_START_BLOCK,
    )
    ...
    const bs = new TrackingFeatureFlags(POINTS_TRACKING_START_BLOCK, ZERO_BI, ZERO_BI, LINEA_TVL_TRACKING_START_BLOCK)
...
export function getVaultFeatureFlags(vault: BeefyVault): TrackingFeatureFlags {
  return new TrackingFeatureFlags(
    vault.startTrackingVaultDataAtBlock,
    vault.startTrackingVaultBalanceBreakdownAtBlock,
    vault.startTrackingInvestorBalanceBreakdownAtBlock,
    vault.startTrackingInvestorTokenBalanceSnapshotsAtBlock,
  )
}

const CONTEXT_KEY_UNDERLYING_PLATFORM = "underlyingPlatform"
const CONTEXT_KEY_VAULT_KEY = "vaultKey"
const CONTEXT_KEY_FEATURE_FLAG_START_TRACKING_VAULT_DATA_AT_BLOCK = "startTrackingVaultDataAtBlock"
const CONTEXT_KEY_FEATURE_FLAG_START_TRACKING_VAULT_BALANCE_BREAKDOWN_AT_BLOCK = "startTrackingVaultBalanceBreakdownAtBlock"
...
export function buildVaultDataSourceContext(vault: VaultConfig): DataSourceContext {
  ...
  context.setBigInt(CONTEXT_KEY_FEATURE_FLAG_START_TRACKING_VAULT_DATA_AT_BLOCK, vault.featureFlags.startTrackingVaultDataAtBlock)
  context.setBigInt(
    CONTEXT_KEY_FEATURE_FLAG_START_TRACKING_VAULT_BALANCE_BREAKDOWN_AT_BLOCK,
    vault.featureFlags.startTrackingVaultBalanceBreakdownAtBlock,
  )
  context.setBigInt(
...
export function getContextFeatureFlags(): TrackingFeatureFlags {
  let context = dataSource.context()
  return new TrackingFeatureFlags(
    context.getBigInt(CONTEXT_KEY_FEATURE_FLAG_START_TRACKING_VAULT_DATA_AT_BLOCK),
    context.getBigInt(CONTEXT_KEY_FEATURE_FLAG_START_TRACKING_VAULT_BALANCE_BREAKDOWN_AT_BLOCK),
...
class TrackingFeatureFlags {
  constructor(
     ...
     */
    public startTrackingVaultDataAtBlock: BigInt,
    public startTrackingVaultBalanceBreakdownAtBlock: BigInt,
    ...
    if (startTrackingVaultDataAtBlock.lt(ZERO_BI)) {
      log.error("startTrackingVaultDataAtBlock: {}", [startTrackingVaultDataAtBlock.toString()])
      throw new Error("startTrackingVaultDataAtBlock should be >= 0")
    }
    if (startTrackingVaultBalanceBreakdownAtBlock.lt(ZERO_BI)) {
      log.error("startTrackingVaultBalanceBreakdownAtBlock: {}", [startTrackingVaultBalanceBreakdownAtBlock.toString()])
      throw new Error("startTrackingVaultBalanceBreakdownAtBlock should be >= 0")
    }
    if (startTrackingInvestorBalanceBreakdownAtBlock.lt(ZERO_BI)) {
      log.error("startTrackingInvestorBalanceBreakdownAtBlock: {}", [startTrackingInvestorBalanceBreakdownAtBlock.toString()])
    ...
    if (startTrackingInvestorTokenBalanceSnapshotsAtBlock.lt(ZERO_BI)) {
      log.error("startTrackingInvestorTokenBalanceSnapshotsAtBlock: {}", [startTrackingInvestorTokenBalanceSnapshotsAtBlock.toString()])
      throw new Error("startTrackingInvestorTokenBalanceSnapshotsAtBlock should be >= 0")
    }

    // vault balance breakdown needs vault data
    if (startTrackingVaultBalanceBreakdownAtBlock.gt(ZERO_BI) && startTrackingVaultDataAtBlock.equals(ZERO_BI)) {
      log.error("Got startTrackingVaultBalanceBreakdownAtBlock: {}, and startTrackingVaultDataAtBlock: {}", [
        startTrackingVaultBalanceBreakdownAtBlock.toString(),
        startTrackingVaultDataAtBlock.toString(),
      ])
      throw new Error("startTrackingVaultBalanceBreakdownAtBlock can't be set without startTrackingVaultDataAtBlock")
    } else if (startTrackingVaultBalanceBreakdownAtBlock.gt(ZERO_BI) && startTrackingVaultBalanceBreakdownAtBlock.lt(startTrackingVaultDataAtBlock)) {
      log.error("Got startTrackingVaultBalanceBreakdownAtBlock: {}, and startTrackingVaultDataAtBlock: {}", [
        startTrackingVaultBalanceBreakdownAtBlock.toString(),
        startTrackingVaultDataAtBlock.toString(),
      ])
      throw new Error("startTrackingVaultBalanceBreakdownAtBlock should be >= startTrackingVaultDataAtBlock")
    }
    // investor balance breakdown needs vault balance breakdown
    if (startTrackingInvestorBalanceBreakdownAtBlock.gt(ZERO_BI) && startTrackingVaultBalanceBreakdownAtBlock.equals(ZERO_BI)) {
      log.error("Got startTrackingInvestorBalanceBreakdownAtBlock: {}, and startTrackingVaultBalanceBreakdownAtBlock: {}", [
        startTrackingInvestorBalanceBreakdownAtBlock.toString(),
        startTrackingVaultBalanceBreakdownAtBlock.toString(),
      ])
      throw new Error("startTrackingInvestorBalanceBreakdownAtBlock can't be set without startTrackingVaultBalanceBreakdownAtBlock")
    } else if (
      startTrackingInvestorBalanceBreakdownAtBlock.gt(ZERO_BI) &&
      startTrackingInvestorBalanceBreakdownAtBlock.lt(startTrackingVaultBalanceBreakdownAtBlock)
    ) {
      log.error("Got startTrackingInvestorBalanceBreakdownAtBlock: {}, and startTrackingVaultBalanceBreakdownAtBlock: {}", [
        startTrackingInvestorBalanceBreakdownAtBlock.toString(),
        startTrackingVaultBalanceBreakdownAtBlock.toString(),
      ])
      throw new Error("startTrackingInvestorBalanceBreakdownAtBlock should be >= startTrackingVaultBalanceBreakdownAtBlock")
    }
    // investor token balance needs vault data
    if (startTrackingInvestorTokenBalanceSnapshotsAtBlock.gt(ZERO_BI) && startTrackingVaultDataAtBlock.equals(ZERO_BI)) {
      log.error("Got startTrackingInvestorTokenBalanceSnapshotsAtBlock: {}, and startTrackingVaultDataAtBlock: {}", [
        startTrackingInvestorTokenBalanceSnapshotsAtBlock.toString(),
        startTrackingVaultDataAtBlock.toString(),
      ])
      throw new Error("startTrackingInvestorTokenBalanceSnapshotsAtBlock can't be set without startTrackingVaultDataAtBlock")
    } else if (
      startTrackingInvestorTokenBalanceSnapshotsAtBlock.gt(ZERO_BI) &&
      startTrackingInvestorTokenBalanceSnapshotsAtBlock.lt(startTrackingVaultDataAtBlock)
    ) {
      log.error("Got startTrackingInvestorTokenBalanceSnapshotsAtBlock: {}, and startTrackingVaultDataAtBlock: {}", [
        startTrackingInvestorTokenBalanceSnapshotsAtBlock.toString(),
        startTrackingVaultDataAtBlock.toString(),
      ])
      throw new Error("startTrackingInvestorTokenBalanceSnapshotsAtBlock should be >= startTrackingVaultDataAtBlock")
    }
  }

  public shouldTrackVaultData(blockNumber: BigInt): boolean {
    return this.shouldTrack(this.startTrackingVaultDataAtBlock, blockNumber)
  }

  public shouldTrackVaultBalanceBreakdown(blockNumber: BigInt): boolean {
    return this.shouldTrack(this.startTrackingVaultBalanceBreakdownAtBlock, blockNumber)
  }
